<?php
	...
	function canSeeProjectedSales($userid = 0)
	{
		$userDetail = getUserDetails($userid);
		$seeprojectedsales = $userDetail['seeprojectedsales'];

		return 1 == intval($seeprojectedsales);
	}

	function canSeeProjectedSalesYear($userid = 0)
	{
		$userDetail = getUserDetails($userid);
		$seeprojectedsalesyear = $userDetail['seeprojectedsalesyear'];

		return 1 == intval($seeprojectedsalesyear);
	}

	function canSeeOrderFulfillment($userid = 0)
	{
		$userDetail = getUserDetails($userid);
		$seeorderfulfillment = $userDetail['seeorderfulfillment'];

		return 1 == intval($seeorderfulfillment);
	}
