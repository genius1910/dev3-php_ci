...
class Users_model extends Model {
  ...
  public function getUsersList($isAdmin=true){			
		...
		if($this->session->userdata('hiddenuser') == 1)
			$this->db->where('hiddenuser', 1);
		else
			$this->db->where('hiddenuser', 0);
...
// End Get Active User //	
  ...
  public function getUsersListDropDown(){
		$isHiddenUser = $this->session->userdata('hiddenuser');
  ...
  public function getUserTargets($userid) {		
	...
	public function getLicences() {
		$this->db->select('goldlicences, silverlicences');
		$this->db->from('system');
        $query = $this->db->get();
        return $query->result_array();
	}
	...
	public function getUserCount($data) {
		...
		$this->db->from('users');
		$this->db->where('licencetype',$data);
        $query = $this->db->get();
        return $query->result_array();
	}
		...
		if($unique) {
			if(isset($data['userid']) && 0!=intval($data['userid'])) {
				if(""!=$type) {
					...
					if($data['active'] == 1)
						$this->checkLicence('add', $data['licencetype'], $data['active']);
					...
					if($this->db->update('users', $data)) {
						if($data['hiddentype'] == '0' && $data['active'] == 1)
							$this->checkLicence('edit', $data['licencetype'], $data['active']);
						$uesrid = $data['userid'];
	...
	public function checkLicence($method, $field)
	{
		$data = $this->db->get('system')->row_array();
		$value[$field == 'G' ? 'goldlicences' : 'silverlicences'] = $field == 'G' ? $data['goldlicences'] + 1 : $data['silverlicences'] + 1;
		if($method == 'edit')
			$value[$field == 'G' ? 'goldlicences' : 'silverlicences'] = $field == 'S' ? $data['goldlicences'] - 1 : $data['silverlicences'] - 1;

		$this->db->where('id', 1);
		return $this->db->update('system', $value);
	}
