	  ...
	  if($this->site_model->is_logged_in()==false){
		...
		if(!empty($submit)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($this->input->post('licencetype'))
					$posted_data['licencetype'] = $this->input->post('licencetype');

				if($this->input->post('defaultmodule'))
					$posted_data['defaultmodule'] = $this->input->post('defaultmodule');

				$message = '';
				//
				$licences = $this->users_model->getLicences()[0];
				$count = $this->users_model->getUserCount($posted_data['licencetype'])[0]['count'];
				$result = $posted_data['licencetype'] == 'G' ? ($licences["goldlicences"] - $count) : ($licences["silverlicences"] - $count);
				//
				if("copy"==$posted_data['type']) {
					...
					if(!empty($posted_data['userid'])) {
						...
						if($canAdd) {
							if($result <= 0){
								$message = '<div class="alert alert-warning">Do not add user! You can not add more because the number is exceeded.';
								$this->session->set_flashdata('user_operation', $message.'</div>');
								redirect('users/index');
								return;
							}
