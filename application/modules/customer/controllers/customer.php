...
class Customer extends Controller
{
	...
	public function GetThreeYearChartCanvas()
	{
		$account = $_POST['account'];
		$data['account'] = base64_decode($account);
		$this->session->set_userdata("myaccount", $data['account']);
	...
	function getPACTable() {
		...
		$data['r_pac'] = $this->makePacList($this->customer_model->getCustomerPACDetailsWithGroupByModified($data['account'], 'pac' . $index, $data['startthisyearrmonth'], $data['curyearmonth'], $proRataCoefficient));
		echo json_encode($data);
	}
	function customerDetails($account)
	{
		...
		if ($data['address1'] <> "")
			$data['nameaddress'] = trim($data['nameaddress']) . ", " . $data['address1'];
		if ($data['address2'] <> "")
			$data['nameaddress'] = trim($data['nameaddress']) . ", " . $data['address2'];
		if ($data['address3'] <> "")
			$data['nameaddress'] = trim($data['nameaddress']) . ", " . $data['address3'];
		if ($data['address4'] <> "")
			$data['nameaddress'] = trim($data['nameaddress']) . ", " . $data['address4'];
		if ($data['address5'] <> "")
			$data['nameaddress'] = trim($data['nameaddress']) . ", " . $data['address5'];
		if ($data['postcode'] <> "")
			$data['nameaddress'] = trim($data['nameaddress']) . ", " . $data['postcode'];
		if ($data['phone'] <> "")
			$data['nameaddress'] = trim($data['nameaddress']) . " p:" . $data['phone'];
		if ($data['email1'] <> "")
			$data['nameaddress'] = trim($data['nameaddress']) . " e:<a href=\"mailto:" . $data['email1'] . "\">" . $data['email1'] . "</a>";
	...
	function customerModelGraph()
	{
		...
		if ($level == "pac1")
			$levelclause = "product.pac1code = '$code'";
		if ($level == "pac2")
			$levelclause = "product.pac2code = '$code'";
		if ($level == "pac3")
			$levelclause = "product.pac3code = '$code'";
		if ($level == "pac4")
			$levelclause = "product.pac4code = '$code'";
		if ($level == "product")
			$levelclause = "product.code = '$code'";
		...
		foreach ($result as $row) {
			...
			for ($x = 0; $x <= 36; $x++) {
				if ($yearmonth[$x] == $salessummaryyearmonth)
					$sales[$x] = $salessummarysales; // If the year month of the data matches the array, put the value in
			}
		}
	...
	public function getCustomergraphtable()
	{
		$account = $_POST['account'];
		$level = $_POST['level'];
		$code = $_POST['code'];
	...
	public function getCustomergraphchart()
	{
		$account = $_POST['account'];
		$level = $_POST['level'];
		$code = $_POST['code'];
