    ...
    var table_prd = $(".product-example-table").DataTable({
      ...
      ajax: {
        ...
        complete: function (resp) {
          searchString = $(
            '#product-example-table_wrapper .dataTables_filter input[type="search"]'
          ).val();
...
$(function () {
  ...
  var table = $("#item_list_table").DataTable({
    processing: true,
    language: {
      processing: `<div class="overlay h2 m-0" style="margin: 0px;display: block;position: absolute;">
											<i class="fa fa-spinner fa-spin fa-fw"></i>
    ...
    order: [[0, "asc"]],
    ajax: {
      url: base_url + "itemList/fetchItemList",
      type: "post",
      complete: function (resp) {
        searchString = $(
          '#item_list_table_wrapper .dataTables_filter input[type="search"]'
        ).val();
        $(".dataTables_processing").css("display", "none");

        $("#item_list_table tr").each(function (k, v) {
  ...
  $("#item_list_table tfoot th").each(function () {
    var title = $(this).text();
    $(this).html(
...
function ProcessingExport(action) {
  if ("show" == action) {
    ...
    notification_html =
      "<div class='notification'>Processing please wait!!</div>";
    ...
    success: function (response) {
      ...
      if (edtID == "discnt") {
        ...
        $(".discnt").on("save", function (e, params) {
          ...
          var terms =
            dscID.indexOf("termsproduct") != -1 ? "termsproduct" : "termsgroup";
...
getTermProduct = function (thisObj, value) {
  ...
  var baseprice2 =
    ...
    parseFloat((parseFloat(baseprice) / 100) * parseFloat(discount1));

  var netprice =
    parseFloat(baseprice2) -
    parseFloat((parseFloat(baseprice2) / 100) * parseFloat(discount2));
  ...
  if (did.indexOf("discount1") != -1) {
    $(thisObj)
      ...
      .next("td")
      .next("td")
      .text(parseFloat(netprice).toFixed(2)); // Put the net price
  } else {
...
function loadCustomerProducts(customersales) {
  $.ajax({
    url:
      base_url +
      "customer/drawCustomerProductsDetails/" +
      customer_account +
      "/" +
      customer_startthisyearrmonth +
      "/" +
      customer_curyearmonth,
...
var closeAdduploadTargetForm = function () {
  return $(".hidden-add-uploadtarget-form")
    .fadeOut("fast", function () {
      $(".overlay").hide();
    })
    .promise();
...
var selectOptionKey = function (e) {
  ...
  if (40 == e.keyCode) {
    if (!!e.target.nextElementSibling) {
      ...
      document
        .querySelector("#searchedcodes ul")
  ...
  if (38 == e.keyCode) {
    if (!!e.target.previousElementSibling) {
      ...
      document
        .querySelector("#searchedcodes ul")
        .lastChild.previousElementSibling.focus();
    }
  }
};
...
var executeAutoComplete = function (evt) {
  ...
  if ("P" !== evt.target.value) {
    ...
    alertElement.innerText =
      "Products list is very large. Please type atleast 2 characters and hit the search icon to search.";
...
var uploadDataSuccess = function () {
  var successMsg =
    "Targets data for the customer is uploaded successfully! Please wait while the data is updated in the table.";
...
var addDataSuccess = function (result) {
  var successMsg =
    "Target data for the customer is added successfully! Please wait while the data is updated in the table.";
...
var adjustTable = function (obj) {
  ...
  $(obj).addClass(
    "table table-bordered table-striped target-list-table target-listing"
  );
};
  ...
  if (!isDiplayed(displayig_id)) {
    if (!!updated) {
      ...
      if ("yearmonth" != editField) {
        document.querySelector("#" + displayig_id).innerText =
          updated.salestarget;
      } else {
        document.querySelector("#" + displayig_id).innerText =
          updated.yearmonth;
...
let isDiplayed = function (element_id) {
...
var codetypetable = [
  "customersalestarget",
  "customerpac1salestarget",
  "customerpac2salestarget",
  "customerpac3salestarget",
...
$(function () {
  $("#item_list_table_filter").after($("#uploadTargetsBulk"));
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if ("success" == response.status) {
        showTaskOperationStatus(
          (statusHidden = false),
          (statusTitle = "Success!"),
          (statusMessage = response.deleted),
    ...
    .then(function (result) {
      ...
      if (200 == result.status) {
        showTaskOperationStatus(
          (statusHidden = false),
          (statusTitle = "Success!"),
          (statusMessage = "The new data is created successfully."),
          (statusClass = "success")
        );
      } else {
        showTaskOperationStatus(
          (statusHidden = false),
          (statusTitle = "Error!"),
          (statusMessage = "Data could not be created."),
          (statusClass = "danger")
        );
      }
...
var populateCustomerRepsTable = function (accountId = null) {
  ...
  var url =
    undefined != user_id
      ? `${base_url}users/repcodes/${user_id}`
      : `${base_url}customer/repcodes/${accountId}`;
  ...
  if (!fullDescriptionVisible) {
    $("#desc-" + id + " .full-desc")
      ...
      .done(function () {
        $("#desc-" + id)
          .next()
          .attr("title", "show short description");
        $("#desc-" + id)
          .next()
          .children(".show-full")
          .html("[show less]");
    ...
    $("#desc-" + id + " .full-desc")
      ...
      .done(function () {
        $("#desc-" + id)
          .next()
          .attr("title", "show full description");
        $("#desc-" + id)
          .next()
          .children(".show-full")
          .html("[show more]");
...
var showTaskOperationStatus = function (
  statusHidden,
  statusTitle,
  statusMessage,
  statusClass
...
var deleteRecord = function (taskid, evt) {
  ...
  var confirmDelete = confirm(
    "Are you sure to delete the task #" + taskid + "?"
  );
  if (!confirmDelete) {
  } else {
...
var deleteFile = function (filepath, filename, id, evt) {
  ...
  var confirmation = confirm(
    "You are processing to delete the file " +
      filename +
...
var getLastUrlSegment = function () {
  var urlArray1 = window.location.href
    .replace("https://", "")
    .replace("http://", "")
    .split("/");
...
var autoCompleteBind = function (callbackFn) {
  ...
  $("#account_selector").blur(function () {
    ...
    $("#contactno").before(
      "<span class='bg-warning' id='contactno_warning'><i class='glyphicon glyphicon-exclamation-sign'></i>Please wait while the contacts for the selected account is being fetched from the database.</span>"
    );
    fetchContacts();
  });
  try {
    if (!jQuery.isEmptyObject($("#account_selector"))) {
      ...
      console.log(
        "autocomplete is bound successfully with the account field now!"
      );
...
var customerRepform = function (evt) {
  ...
  if ("" == $.trim(repcode)) {
    var errorSpanElem = errorSpan(
      "repcodeError",
      "<i class='fa fa-fw fa-exclamation' aria-label='error'></i> The repcode field can not be blank"
    );
